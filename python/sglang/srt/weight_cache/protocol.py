...
def check_ipc_quant_support(
    quant_method: str, quant_config: Any, *, where: str, model_config: Any = None
) -> None:
    ...
    """
    # Mixed checkpoints mark MXFP4 routed experts on the model config, not in
    # hf quantization_config, so they would otherwise bypass the method gate.
    if model_config is not None and model_config.is_fp4_experts:
        raise UnsupportedQuantForIPCError(
            f"[weight_cache:{where}] MXFP4 routed experts "
            f"(model_config.is_fp4_experts) are not verified for CUDA IPC "
            f"zero-copy weight sharing: post-load processing repacks expert "
            f"weights and scales into an interleaved layout the "
            f"meta-initialized client cannot reproduce. Disable the weight "
            f"cache (--weight-cache-mode off) for this model."
        )
